...
def issue():
    ...
    global issueBtn,labelFrame,lb1,inf1,inf2,quitBtn,root,Canvas1,status,status_2
    ...
    extractMid = "select memid from "+memTable
    try:
        cur.execute(extractMid)
        con.commit()
        for i in cur:
            allMid.append(i[0])
            
        if issueto in allMid:
            status_2 = True
    ...
    show = "select * from "+issueTable  ## debug purpose
    
    updateStatus = "update "+bookTable+" set status = 'issued' where bookid = '"+bid+"'"
   
    try:
        if bid in allBid and  issueto in allMid and status == True and status_2 == True:
            ...
            allBid.clear()
            messagebox.showinfo('Message',"Book Already Issued OR member not registered")
            root.destroy()
            return
    except:
        messagebox.showinfo("Search Error","Some value entered is wrong, Try again")
...
def issueBook(): 
    ...
    global issueBtn,labelFrame,lb1,inf1,inf2,quitBtn,root,Canvas1,status_2,status
