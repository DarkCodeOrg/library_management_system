...
def searchBook():
    ...
    bookname = bookInfo1.get()
    bookname = bookname.upper()
    
    searchSql = "select bookid,title from "+bookTable+" where title like '%"+bookname+"%'"
    ...
    headingLabel = Label(headingFrame1, text="Search Books", bg='yellow', fg='blue', font=('Courier',15))
    ...
    Label(labelFrame, text="%-10s%-40s"%('Book ID','Title'),bg='black',fg='white').place(relx=0.07,rely=0.1)
...
def search1():
    ...
    #Submit Button
    SubmitBtn = Button(root,text="SUBMIT",bg='#d1ccc0', fg='black',command=searchBook)
